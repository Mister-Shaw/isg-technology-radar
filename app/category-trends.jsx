...
import {categoryMonthlyCsv} from '@/lib/monthly';
const metrics={total_amount:'合并披露金额',events:'收录独立事件'};
const fmt=n=>n==null?'—':n.toLocaleString('zh-CN',{maximumFractionDigits:2});
export default function CategoryTrends({window,matrix:allRows,category,month,onSelect,download,onEdit}){
 const w=dateWindow(window),months=monthsInWindow(w),[metric,setMetric]=useState('total_amount'),[level,setLevel]=useState('parent'),definitions=level==='parent'?PARENTS:CHILDREN,matrix=allRows.filter(r=>Object.hasOwn(definitions,r.category)),money=metric==='total_amount',colors=panelColors([{points:matrix.map(r=>({value:r[metric]??null}))}]);
