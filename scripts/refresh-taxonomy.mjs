...
const panel=read('public/market-panel.json'),research=read('data/research-2026.json');
...
const taxonomy={version:TAXONOMY_VERSION,reclassified_at:process.argv.find(a=>/^\d{4}-\d{2}-\d{2}$/.test(a))||new Date().toISOString().slice(0,10),groups:GROUPS,children:CHILDREN,rules:[TAXONOMY_NOTE,XPU_NOTE,'固定媒体全部新闻为分母；各月份统一标题识别；只重算分类，未新增采集。','海光或鲲鹏备选同时计入路线提及，不能理解为均已采用。父类并集去重。'],xpu_search_status:'历史资料重识别；未完成专项补采',application_new_topics:'海光、鲲鹏、xPU标题行为复核未完成',procurement_new_topics:'海光、鲲鹏、xPU适用分母未建立'};
...
panel.taxonomy=taxonomy;write('public/market-panel.json',panel);
const sources=read('public/sample-sources.json');sources.taxonomy=taxonomy;sources.sources=panel.sources.filter(s=>['media','official'].includes(s.group)&&!s.document_source_id);write('public/sample-sources.json',sources);
...
write('public/taxonomy-review.json',review);
console.log(JSON.stringify({version:TAXONOMY_VERSION,records:researchRows.length,documents:panel.documents.length,topics:Object.fromEntries(Object.entries(review.topics).map(([k,v])=>[k,{news:v.news_ids.length,research:v.research_ids.length}]))}));
