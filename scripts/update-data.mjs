...
export function rereadStart(window,end){
 let start=day(window.end,-13);
 // First completed week of a month also revisits the preceding calendar month.
 if(Number(end.slice(8))<=7)start=[start,day(end.slice(0,7)+'-01',-1).slice(0,7)+'-01'].sort()[0];
 return start<window.start?window.start:start;
}
...
export function mergeMedia(original,fresh){
 ...
 assert(start<=end&&end<=fresh.scanned_through&&start<=day(oldEnd,1)&&end>=oldEnd,'Collection must cover the gap without moving backwards');
 ...
 for(const id of MEDIA){
  ...
  const prior=original.documents.filter(d=>d.source_id===id&&d.date>=start&&d.date<=oldEnd).length;
  const reread=new Set(fresh.documents.filter(d=>d.source_id===id&&d.date>=start&&d.date<=oldEnd).map(d=>d.id)).size;
  assert(!prior||reread>=prior*.5,`${id}: reread returned less than half the existing same-window archive; check missing pages before applying`);
 }
 ...
 for(const s of panel.sources.filter(s=>MEDIA.includes(s.id))){
  s.coverage=months.map(month=>{
   const old=s.coverage.find(c=>c.month===month),from=month+'-01';
   const continuous=old?.status==='complete'&&month<=oldEnd.slice(0,7);
   const fullReread=start<=(from<panel.window.start?panel.window.start:from);
   const rows=panel.documents.filter(d=>d.source_id===s.id&&d.month===month&&d.date>=panel.window.start&&d.date<=end);
   return {month,status:continuous||fullReread?'complete':'incomplete',total:rows.length,eligible:rows.filter(d=>!d.superseded).length,covered_through:month===end.slice(0,7)?end:day(day(month+'-28',4).slice(0,7)+'-01',-1),note:`全部新闻为分母；同源标题去重；统计截至${end}，完整仅指当前可访问目录遍历。`};
  });
  s.last_rechecked_at=fresh.checked_at;s.recheck_window={start,end};
  s.limitation=`自动回查${start}至${end}；全部新闻保留在分母。历史删除、异标题转载及来源结构变化仍可能影响结果；新增标题的采购/部署行为尚待复核。`;
 }
 ...
 if(end>oldEnd)for(const s of panel.sources.filter(s=>!MEDIA.includes(s.id)&&!s.document_source_id&&!s.document_source_ids)){
  for(const c of s.coverage||[])if(c.month>=oldEnd.slice(0,7)&&c.month<=end.slice(0,7)){
   ...
   c.status='incomplete';c.covered_through=oldEnd;c.note=`本次仅更新媒体；该来源尚未补采${day(oldEnd,1)}之后的数据。`;
  }
 }
...
function main(){
 ...
 try{
  ...
  const end=options['--end']||lastSunday(),start=rereadStart(original.window,end);
  ...
  if(options['--input'])fresh=read(path.resolve(options['--input']));
   ...
   console.log(`Collecting all news: ${start} through ${end}; batch ${id}`);
   const result=spawnSync(options['--python']||process.env.PYTHON||'python',['-X','utf8',path.join(root,'tools/collectors/refresh-media.py'),'--start',start,'--end',end,'--cutoff',end,'--output',`output/${id}`],{cwd:root,stdio:'inherit',timeout:15*60*1000,windowsHide:true});
  ...
  assert.equal(fresh.cutoff,end,'Input cutoff differs from --end');assert(fresh.reread_start<=start,'Input must include the required lookback');
  ...
  const files={'public/market-panel.json':taxonomy.panel,'public/sample-sources.json':taxonomy.sources,'public/taxonomy-review.json':taxonomy.review,'data/weekly-monitor.json':{...config,last_completed_at:fresh.checked_at,last_completed_cutoff:end,latest_window:monitor.window,latest_status:`固定四媒体已自动更新至${end}；新增${summary.added_rows}篇，${candidates.length}条标题线索待核验，月度研究与金额未自动改写。`,latest_alerts:monitor.alerts},'weekly-runs/last-success.json':monitor.next_state};
  ...
  console.log(JSON.stringify({applied:!options['--dry-run'],end,added:summary.added_rows,changed:summary.changed_rows,candidates:candidates.length,alerts:monitor.alerts.length,report:path.join(output,'report.json')}));
