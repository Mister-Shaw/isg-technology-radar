...
第一条联网采集并检查，不写回数据；第二条按“来源×月份”写入通过检查的数据，异常月份保留旧记录及实际成功截止；没有旧数据则标记缺口。自动读取 DOIT、C114、至顶、电脑商情四个固定媒体的全部新闻，去重、分类，同步市场风向与样本来源，并生成异动报告。每轮都从研究起始日（附带快照为2026年1月1日）全量回查至北京时间最近一个完整周日；跨年仍保留原起点，不依赖本机是否执行过往期更新。

每轮自动延伸尝试窗口、月份列表、热力表列数和来源覆盖表，无需手动把9个月改为10个月；跨年按年月区分。各来源月份单独记录实际覆盖截止；只有全批通过才推进完整成功截止。未补采的项目研究、官网和采购来源保留各自真实截止。

脚本保留固定四媒体的全部新闻作分母，不因某家失败而缩小分母。覆盖不齐的月份不显示跨来源比较比例，完整月份与成功来源仍可查看；新标题的采购/部署表述未经人工复核时，相应月份的行为指标显示待复核。新项目线索输出到 `weekly-runs/<批次>/research-candidates.json`，金额和研究台账需核验后导入；本地数据库及笔记不会被改写。

退出码 `0` 表示完整完成，`2` 表示部分成功（普通运行已写回通过的月份，预演仅报告），`1` 表示结构、执行或写回失败。失败月份本轮读到但未采用的记录保存在 `deferred-news.json`，不替换旧资料。支持重复运行、备份及中断恢复。运行记录在 `weekly-runs/`，采集明细在 `tools/collectors/output/`，均不提交 Git。构建版本更新后需重新 `npm run build` 并重启；开发服务刷新页面即可读取新媒体数据。
