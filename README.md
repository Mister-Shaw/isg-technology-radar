...
第一条联网采集并检查，不写回数据；第二条检查通过后更新本地数据文件。自动读取 DOIT、C114、至顶、电脑商情四个固定媒体的全部新闻，去重、分类，同步市场风向与样本来源，并生成异动报告。每轮都从研究起始日（附带快照为2026年1月1日）全量回查至北京时间最近一个完整周日；跨年仍保留原起点，不依赖本机是否执行过往期更新。

成功后自动延伸媒体截止日期、月份列表、热力表列数和来源覆盖表，无需手动把9个月改为10个月；跨年按年月区分。未补采的项目研究、官网和采购来源保留各自真实截止。
