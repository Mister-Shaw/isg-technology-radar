...
export function coverageComplete(source,month,end){
 const c=source?.coverage?.find(c=>c.month===month);
 return c?.status==='complete'&&(c.covered_through===undefined||c.covered_through>=end);
}
...
export function panelCell(data,source,category,metric,period,chip='all'){
 ...
 const coverageSources=source.document_source_ids?source.document_source_ids.map(id=>data.sources.find(s=>s.id===id)):[source];
 const covered=period.months.every(month=>{const end=[w.end,new Date(Date.UTC(Number(month.slice(0,4)),Number(month.slice(5)),0)).toISOString().slice(0,10)].sort()[0];return coverageComplete(source,month,end)&&coverageSources.every(s=>coverageComplete(s,month,end));});
 ...
 const complete=established&&behaviorReviewed&&!behaviorPending&&covered&&(metric!=='application'||source.application_reviewed!==false);
 ...
 return {...period,category,n,N,u,complete,partial:period.partial??period.months.some(m=>partialMonth(m,w)),value:usable?1000*n/N:null,upper:usable?1000*(n+u)/N:null,unknown:usable?1000*u/N:null,other:usable?1000*(N-n-u)/N:null,status:!established?'适用分母未建立':!behaviorReviewed||behaviorPending?'行为复核未完成':!complete?(coverage.some(c=>c?.status==='stale')||coverageSources.some(s=>s?.coverage?.some(c=>period.months.includes(c.month)&&c.status==='stale'))?'含旧数据，暂不可比':'覆盖不完整，暂不可比'):!N?'无适用分母':u?'含未判定条款':'可计算',ids:numerator.map(r=>r.id),denominator_ids:denominator.map(r=>r.id),unknown_ids:unknown.map(r=>r.id)};
}
