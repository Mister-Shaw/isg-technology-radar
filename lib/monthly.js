...
export function combinedAmountSummary(rows){
 const sum=key=>rows.length?rows.reduce((n,e)=>n+Math.round(e[key]*100),0)/1e6:null;
 return {total_amount:sum('_amount_min'),total_amount_upper:sum('_amount_max'),total_amount_count:rows.length};
}
export function categoryMonthRows(rows,leads=[],eligible=amountRows(rows),reported=reportedAmountRows(rows),window,combined=combinedAmountRows(rows),definitions={...TECHS,focus:'常规芯片采购补充'}){
 return Object.entries(definitions).flatMap(([category,label])=>{
  ...
  return monthlySeries(r,'award',eligible,window).map((s,i)=>{const merged=combined.filter(e=>belongs(e)&&monthOf(e)===s.month);return {category,label,month:s.month,events:s.events,projects:s.projects,records:r.filter(e=>monthOf(e)===s.month).length,verified:r.filter(e=>monthOf(e)===s.month&&researchReview(e)==='accepted').length,leads:l.filter(e=>monthOf(e)===s.month).length,...combinedAmountSummary(merged),amount_ids:merged.map(e=>e.id),...Object.fromEntries(Object.entries(amounts).flatMap(([k,v])=>[[k,v[i].amount],[`${k}_count`,v[i].amount_count]]))};});
 });
}
